...
export function LogbookDialog({ open, onOpenChange, editRecord }: Props) {
  ...
  const canApprove = hasPermission("lab_sessions.edit");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.course_name || !form.start_time) return toast.error("Course/activity name and start time are required");
    setLoading(true);
    const payload: any = {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>{isEdit ? "Edit Log Entry" : "Add New Log"}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-2 rounded-md border border-border bg-muted/30 p-3">
            ...
            <Label htmlFor="approved" className="cursor-pointer text-sm">
              Supervisor / Instructor Approved
              {!canApprove && <span className="ml-2 text-xs text-muted-foreground">(requires approval permission)</span>}
            </Label>
          </div>
          <div className="flex justify-end gap-2">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
            <Button type="submit" disabled={loading}>{loading ? "Saving..." : isEdit ? "Save Changes" : "Add Log"}</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
