  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {canCreate && <Button size="sm" onClick={openAdd}><Plus className="mr-2 h-4 w-4" /> Log Activity</Button>}
      </div>
              ...
              <tr key={a.id} className="hover:bg-muted/30">
                ...
                {showActions && (
                  <td className="px-4 py-2 text-right">
                    <RowActions table="technician_activities" id={a.id} invalidateKey="technician_activities" canEdit={canEdit} canDelete={canDelete} onEdit={() => openEdit(a)} itemLabel="activity" />
                  </td>
                )}
              </tr>
            ))}
      ...
      <TechnicianActivityDialog open={dialogOpen} onOpenChange={setDialogOpen} editRecord={editRecord} />
    </div>
  );
}
